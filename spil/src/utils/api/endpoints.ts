export const API_ENDPOINTS = {
  ATTACHMENTS: 'attachments/images',
  PRODUCT_ATTACHMENTS: 'attachments/product/images',
  CATEGORY_ATTACHMENTS: 'attachments/category/images',
  TAG_ATTACHMENTS: 'attachments/tag/images',
  TYPE_ATTACHMENTS: 'attachments/type/images',
  USER_ATTACHMENTS: 'attachments/user/images',
  ANALYTICS: 'analytics',
  ATTRIBUTES: 'attributes',
  ATTRIBUTE_VALUES: 'attribute-values',
  ORDER_STATUS: 'order-status',
  ORDERS: 'orders',
  USERS: 'users',
  REGISTER: 'register',
  PRODUCTS: 'products',
  POPULAR_PRODUCTS: 'products',
  COUPONS: 'coupons',
  CUSTOMERS: 'customers',
  TAXES: 'taxes',
  SHIPPINGS: 'shippings',
  SETTINGS: 'settings',
  CATEGORIES: 'categories',
  ALL_CATEGORIES: 'categories/all',
  TAGS: 'tags',
  // PARENT_CATEGORIES: "fetch-parent-category",
  PARENT_CATEGORIES: 'categories',
  TYPES: 'types',
  PROFILE_UPDATE: 'profile-update',
  LOGOUT: 'logout',
  // ME: 'me',
  ME: 'accounts/info',
  TOKEN: 'accounts/authenticate',
  BLOCK_USER: 'users/ban-user',
  UNBLOCK_USER: 'users/active-user',
  CHANGE_PASSWORD: 'change-password',
  FORGET_PASSWORD: 'forget-password',
  VERIFY_FORGET_PASSWORD_TOKEN: 'verify-forget-password-token',
  RESET_PASSWORD: 'reset-password',
  DOWNLOAD_INVOICE: 'download/invoice',
  APPROVE_SHOP: 'shops/approve-shop',
  ANSWER_QUESTION: 'questions',
  DISAPPROVE_SHOP: 'disapprove-shop',
  SHOPS: 'shops',
  MY_SHOPS: 'my-shops',
  WITHDRAWS: 'withdraws',
  QUESTIONS: 'questions',
  REVIEWS: 'reviews',
  APPROVE_WITHDRAW: 'approve',
  STAFFS: 'staffs',
  ADD_STAFF: 'staffs',
  REMOVE_STAFF: 'staffs',
  IMPORT_PRODUCTS: 'import-products/',
  IMPORT_ATTRIBUTES: 'import-attributes/',
  IMPORT_VARIATION_OPTIONS: 'import-variation-options/',
};
